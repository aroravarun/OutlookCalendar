package com.outlook.calendar.calendarview.utils;
...
import com.outlook.calendar.R;
import com.outlook.calendar.Utils;
 ...
 */
public class CalendarViewLegacyDelegate extends CalendarViewCustom.AbstractCalendarViewDelegate {
    /**
    ...
    private static final int DEFAULT_SHOWN_WEEK_COUNT = 2;
     ...
     */
    private CalendarViewCustom.OnDateChangeListener mOnDateChangeListener;
    ...
    CalendarViewLegacyDelegate(CalendarViewCustom delegator, Context context, AttributeSet attrs,
        ...
        if (!Utils.parseDate(minDate, mMinDate)) {
            Utils.parseDate(DEFAULT_MIN_DATE, mMinDate);
        }
        final String maxDate = a.getString(R.styleable.CalendarView_maxDate);
        if (!Utils.parseDate(maxDate, mMaxDate)) {
            Utils.parseDate(DEFAULT_MAX_DATE, mMaxDate);
        }
        ...
        mSelectedWeekBackgroundColor = a.getColor(
                R.styleable.CalendarView_selectedWeekBackgroundColor, Color.BLUE);
        mFocusedMonthDateColor = a.getColor(
                R.styleable.CalendarView_focusedMonthDateColor, Color.GRAY);
        mUnfocusedMonthDateColor = a.getColor(
                R.styleable.CalendarView_unfocusedMonthDateColor, Color.GRAY);
        ...
        mListView = (ListView) mDelegator.findViewById(R.id.list_view);
//        mDayNamesHeader = (ViewGroup) content.findViewById(R.id.day_names);
    ...
    @Override
    public void setOnDateChangeListener(CalendarViewCustom.OnDateChangeListener listener) {
    ...
    private void updateDateTextSize() {
        ...
        mDateTextSize = Utils.dpToPx(DEFAULT_DATE_TEXT_SIZE, mContext);
...
//        mDayNamesHeader.invalidate();
    ...
    private void goTo(Calendar date, boolean animate, boolean setSelected,
        ...
        if (position < firstFullyVisiblePosition || position > lastFullyVisiblePosition
            ...
            boolean scrollingUpWards = position < firstFullyVisiblePosition;
...
//            }
            mListScrollTopOffset = mShownWeekCount;
            mPreviousScrollState = AbsListView.OnScrollListener.SCROLL_STATE_FLING;
            if (animate) {
                if(scrollingUpWards){
                    mListView.smoothScrollToPositionFromTop(position, 0, GOTO_SCROLL_DURATION);
                }else{
...
//                    mListView.smoothScrollToPositionFromTop(position, 0);
                    ...
                    mListView.setSelection(position-mShownWeekCount+1);
//                    mListView.smoothScrollToPositionFromTop(position, mListScrollTopOffset);
    ...
    private class ScrollStateRunnable implements Runnable {
        ...
        public void run() {
            Log.d("Varun","doScrollStateChange" );
            ...
            if (mNewState != AbsListView.OnScrollListener.SCROLL_STATE_IDLE && mPreviousScrollState == AbsListView.OnScrollListener.SCROLL_STATE_IDLE && !isOverlayMode) {
                isOverlayMode = true;
                mAdapter.notifyDataSetChanged();
                increaseLayoutHeight();
                mShownWeekCount = 5;
                Log.d("Varun","setting isOverlayMode");
            } else if (mNewState == AbsListView.OnScrollListener.SCROLL_STATE_IDLE && mPreviousScrollState != AbsListView.OnScrollListener.SCROLL_STATE_IDLE) {
                Log.d("Varun","reset isOverlayMode");
                isOverlayMode = false;
                mAdapter.notifyDataSetChanged();
            }
...
//            notifyDataSetChanged();
    ...
    private class WeekView extends View {
        ...
        public void init(int weekNumber, int selectedWeekDay, int focusedMonth) {
            ...
            weekNumberOfMonth =  mTempDate.get(Calendar.WEEK_OF_MONTH);
...
//                final boolean isFocusedDay = true;
        ...
        private void drawOverlay(Canvas canvas){
            Log.d("Varun", "drawOverlay: "+isOverlayMode);
            if(!isOverlayMode){
                return;
            }
            Rect rt = new Rect(0, 0, getWidth(), getHeight());
            mOverlayPaint.setStyle(Paint.Style.FILL);
            canvas.drawRect(rt, mOverlayPaint);

        }

        private void drawOverlayText(Canvas canvas){
            Log.d("Varun", "drawOverlayText: "+isOverlayMode);
            if(!isOverlayMode){
                return;
            }
            if(weekNumberOfMonth == 2){
                Paint textPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
                textPaint.setARGB(255, 0, 0, 0);
                textPaint.setTextAlign(Paint.Align.CENTER);
                textPaint.setTextSize(Utils.dpToPx(16, getContext()));
                String text = mTempDate.getDisplayName(Calendar.MONTH, Calendar.LONG, Locale.getDefault());
                int xPos = (canvas.getWidth() / 2);
                int yPos = (int) ((canvas.getHeight() / 2) - ((textPaint.descent() +
                        textPaint.ascent()) / 2)) + 20;
                Rect r = new Rect();
                mOverlayPaint.getTextBounds(text, 0, text.length(), r);
...
//            }
//            canvas.drawRect(mTempRect, mDrawPaint);
            mTempRect.left = mSelectedLeft+5;
            mTempRect.right = mSelectedRight-5;
            float centerX = mTempRect.left + (float)(mTempRect.right - mTempRect.left)/2;
            float centerY = mTempRect.top + (float)(mTempRect.bottom - mTempRect.top)/2;
            canvas.drawCircle(centerX,centerY,(mTempRect.right - mTempRect.left)/2, mDrawPaint);
        }
